...
import { AddressCopy, Button, ChatRoom, WalletConnecButton } from '@/components-new';
    ...
    <div>
      ...
      <div>
        ...
        <ul>
          {users.map((user) => (
            <li key={user.address}>
              ...
              <h2>Chat with {user.name.substr(0, 4)}...{user.name.substr(user.name.length - 4, user.name.length)}</h2>
        ...
        <div className="py-[40px]">
          {currentUser && recipient && <ChatRoom currentUser={currentUser} recipient={recipient} />}
        </div>
      </div>
    </div>
  );
