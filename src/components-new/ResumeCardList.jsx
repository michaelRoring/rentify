...
const ResumeCardList = () => {
  ...
  const { contract, isLoading: isLoadingContract } = useContract(process.env.NEXT_PUBLIC_CONTRACT);
  const { data: getAllGPUs, isLoading: isLoadingGPUs } = useContractRead(contract, "getAllGPUs");
  ...
  useEffect(() => {
    const getRentStats = async () => {
      try {
        ...
        const {  averageRentCost, averageRentDuration, totalNodes, totalRentDuration } = data
  ...
  const totalRent = getAllGPUs?.filter(item => item.renter != ethers.constants.AddressZero)?.length || 0;
