...
const ChatItem = (props) => {
  ...
  useEffect(() => {
    ...
    messagesRef.on('value', (snapshot) => {
      ...
      const onlyMessageOwn = messagesList?.filter(item => item.sender == props.address)
      ...
      <button className="w-full flex items-center px-[10px] py-[10px] focus:bg-[#0D737350] rounded-[6px] gap-[10px]">
        ...
        <div>
          <div className="flex justify-start items-center gap-[10px]">
            <div>
              {!props.currentUser 
                ? addressShort(props.address)
                : 'You'
              }
            </div>
            <div className="w-[3px] h-[3px] rounded-full bg-[#ffffff50]"></div>
            <div className="px-[6px] py-[2px] text-[12px] font-inter rounded-full bg-[#01FF6120] text-[#01FF61]">Level 1</div>
          </div>
          <div className="flex flex-col text-[14px] text-[#ffffff70] font-inter">
            <div className="text-left grow">Joined {moment(props.created_at).fromNow()}</div>
          </div>
        </div>
      </button>
    </li>
          ...
          <div className="flex flex-col gap-[16px] justify-end p-[16px] overflow-y-auto">   
            {messages.map((message, index) => (
              <div key={index} className={`
                ...
                p-[20px] 
                !shadow-sm
                w-[60%] ${address == message.sender 
                  ? 'self-end !rounded-br-[0px] bg-[#0D7373] text-white' : 'self-start !rounded-tl-[0px] bg-black/30'
                }
              `}>
...
const ChatMessage= ({currentUser, sender, avatar, text, timestamp}) => {
  const messageFrom = currentUser == sender 
    ? 'ml-auto rounded-tr-[0] bg-[#0D7373]'
    : 'mr-auto rounded-tl-[0] bg-black/70';
  
  const positionImage = currentUser == sender 
    ? 'order-none'
    : '-order-1';
      ...
      `}>
        <div className="text-[12px] text-white/50 font-semibold mb-[10px]">
          {currentUser == sender ?
            <div>You</div> :
            <div>
              From: <span className="underline cursor-pointer">{addressShort(sender)}</span>
            </div>
          }
        </div>
...
const ChatGroupDetails = (props) => {
  ...
  const handleSendMessage = (newMessage) => {
    if (newMessage.trim() !== '') { // Memastikan recipient sudah terdefinisi
      ...
      <div className="flex flex-col">
        <div className="px-[16px] py-[10px] flex items-center gap-[10px] border-b border-white/20">
          <div className="w-[40px] h-[40px] rounded-full bg-white overflow-hidden">
            <Image src="https://ordexhaust.com/ipfs/uploads/rentify_logo.svg" alt="Logo" width={40} height={40}/>
          </div>
          <div>
            <div className="font-semibold">Rentify Community Chat</div>
            <div className="text-[12px] text-white/50">{props.memberCount} members</div>
          </div>
        </div>
...
const NoConnectWallet = () => {
  return (
    ...
    ">
      ...
      <div className="text-[24px] font-inter font">You must connected wallet first</div>
      <WalletConnecButton />
    </div> 
  )
}
    ...
    <div className="w-[calc(100%_-_365px)] p-[16px]">
      ...
      <div className="flex flex-col gap-[20px] py-[16px]">
        ...
        <div>
          ...
          <div>
            {data.address == props.address ? '[You]' : data.address}
          </div>
        </div>
        <div>
