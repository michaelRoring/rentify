...
const MyGPUList = ({contract, address, myOrder, ...props}) => {
  const { data: getAllGPUs, isLoading } = useContractRead(contract, "getAllGPUs");
  ...
  useEffect(() => {
    ...
    const getMarketPrice = async () => {
      ...
      try {
        const {data} = await axios.get(`/api/market-price?from=${convertFrom}&to=${convertTo}`);
  ...
  const formatGPUsData = getAllGPUs?.map((item, index) => ({
    ...
    rentPrice: [
      {period: 'day', amount: ethers.utils.formatEther(item.rentPrice.day.toString())},
      {period: 'week', amount: ethers.utils.formatEther(item.rentPrice.week.toString())},
      {period: 'month', amount: ethers.utils.formatEther(item.rentPrice.month.toString())},
    ],
    owner: item.owner,
    renter: item.renter,
    ...
    <>
      {isLoading ?
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-[20px] mb-[40px]">
          {myOrder ?
            <>
              {formatGPUsData.filter(({available, owner, renter}) =>
                  !available &&
                  owner == address ||
                  renter == address
                ).map((item, index) =>
