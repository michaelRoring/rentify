...
export function sendRentTemplate(
  customerName,
  ...
  return (`
    <div>
			...
			<br>
			<br>
      <div>We are pleased to inform you that the rental of your item at Rentify platform has been successful. Here are the details:</div>
			<br>
      <div>Name: ${item.name}</div>
      <div>Type: ${item.types}</div>
      <div>vCPUs: ${item.vCPUs} vCPUs [${item.vCPUsUnit} unit]</div>
      <div>Bandwith: ${item.bandwidth} TB</div>
      <div>Duration: ${rentDuration}</div>
      <div>Price: ${item.rentPrice} ${item.symbol.symbolFrom}</div>
			<br>
      <div>We will process your rent within the next 24 hours. Here's your proof of transaction:</div>
